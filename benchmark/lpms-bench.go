...
func main() {
	...
	*/
	...
	tdurCh := make(chan float64, conc*segs)
	...
	for i := 0; i < conc; i++ {
		...
		go func(k int, wg *sync.WaitGroup) {
			...
			for j, v := range pl.Segments {
				...
				tdur := end.Sub(t).Seconds()
				// fmt.Printf("%s,%d,%d,%0.2v\n", end.Format("2006-01-02 15:04:05.999999999"), k, j, tdur)
				if err != nil {
					panic(err)
				}
				tdurCh <- tdur
	...
	totalTDur := 0.0
	for i := 0; i < conc*segs; i++ {
		totalTDur += <-tdurCh
	}
	fmt.Fprintf(os.Stderr, "Total transcode time %v for %v streams\n", totalTDur, conc)
}
