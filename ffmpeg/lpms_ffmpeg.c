...
#define em_err(msg) { \
  ...
  // add video encoder if a decoder exists and this output isn't dropped
  if (ictx->vc && !octx->dv) {
  ...
  // add audio encoder if a decoder exists and this output isn't dropped
  if (ictx->ac && !octx->da) {
...
#define filters_err(msg) { \
  ...
  goto init_video_filters_cleanup; \
    ...
    if (octx->dv) return 0; // dropping video, no need for filters
...
#define main_err(msg) { \
  ...
  while (1) {
    ...
    for (i = 0; i < nb_outputs; i++) {
      ...
      if (ist->index == ictx.vi && !octx->dv) {
        ost = octx->oc->streams[0];
        if (ictx.vc) {
          encoder = octx->vc;
          filter = &octx->vf;
        }
      } else if (ist->index == ictx.ai && !octx->da) {
      ...
      if (!encoder && ost) {
        // stream copy
        AVPacket *pkt = av_packet_clone(&ipkt);
