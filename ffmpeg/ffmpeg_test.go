...
func TestTranscoder_PNGDemuxerOpts(t *testing.T) {
	...
	run, dir := setupTest(t)
	defer os.RemoveAll(dir)
	// generate 3 png frames
	cmd := `
		ffmpeg -i $1/../transcoder/test.ts -an -frames:v 3 test-%d.png

		# Run an ffmpeg CLI equivalent for this PNG demuxer path.
		ffmpeg -framerate 1/3 -f image2 -i test-%d.png -an \
			-vf "fps=30/1,scale=256:144" -c:v libx264 -pix_fmt yuv420p \
			-f mpegts -y ffmpeg-equivalent.ts

		# Check packet count from ffprobe.
		ffprobe -v warning -count_packets -show_streams -select_streams v ffmpeg-equivalent.ts | grep nb_read_packets=270
	`
	run(cmd)
	// 1 frame every 3 seconds, with 3 frames == 9 seconds
	...
	assert.Equal(t, 270, res.Encoded[0].Frames) // 9 seconds * 30fps == 270
}
