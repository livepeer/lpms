...
int open_video_decoder(input_params *params, struct input_ctx *ctx)
{
  ...
  if (ctx->dv) ; // skip decoding video
    ...
    if (AV_HWDEVICE_TYPE_CUDA == params->hw_type) {
	  ...
	  printf("params->video.name %s, params->device %s, params->xcoderParams %s\n", params->video.name, params->device, params->xcoderParams);
    ...
    if (params->hw_type > AV_HWDEVICE_TYPE_NONE && params->hw_type != AV_HWDEVICE_TYPE_MEDIACODEC) {
